...
- **Aggregation**: `GET /api/analytics/summary?days=7|30` (auth-only) returns time series, top pages, device + referrer breakdown, and summary metrics. `visitorChangePct` compares the range vs the immediately preceding equal-length range.
