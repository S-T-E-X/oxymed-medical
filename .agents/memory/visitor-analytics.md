...
- **Quote conversion funnel**: a successful quote-form submit fires `trackInteraction("Teklif Formu Gönderildi")` (a click event, consent-gated). Summary reports `quoteCtaClicks` (clicks with label prefix `"Teklif Al"`), `quoteConversions` (label = the conversion string), and `quoteConversionRate` (round(conv/cta*100), 0 when no clicks). The conversion label is EXCLUDED from `topInteractions` so it doesn't muddy the "most clicked" list — keep the conversion-label constant identical on client (QuotePage) and server (analytics route) or the funnel silently breaks.
