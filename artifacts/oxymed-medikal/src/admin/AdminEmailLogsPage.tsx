...
import { Mail, CheckCircle, XCircle, RefreshCw, Filter } from "lucide-react";
    ...
    <section className="px-4 py-7 sm:px-6 lg:px-8">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        {Object.entries(EMAIL_TYPE_LABELS).map(([val, label]) => (
          <button
            key={val}
            onClick={() => handleFilterChange(typeFilter === val ? "" : val)}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border-2 border-dashed border-slate-200 py-20 text-center">
          ...
          <p className="font-semibold text-slate-400">
            {typeFilter ? "Bu filtreye uyan e-posta logu yok" : "Henüz e-posta gönderilmemiş"}
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-500">Gönderim Zamanı</th>
                ...
                <th className="hidden px-4 py-3 text-left text-xs font-bold text-slate-500 md:table-cell">İlgili Belge</th>
                <th className="hidden px-4 py-3 text-left text-xs font-bold text-slate-500 lg:table-cell">Gönderen</th>
                ...
                <>
                  ...
                  {expandedError === log.id && log.errorMessage && (
                    <tr key={`err-${log.id}`} className="bg-red-50">
                      <td colSpan={6} className="px-4 py-3 text-xs text-red-700 font-mono">
