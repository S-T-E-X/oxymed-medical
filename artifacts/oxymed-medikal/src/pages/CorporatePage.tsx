...
// fallbacks used only when the CMS has no `about` section stored. They stay in
// Turkish by design — translated body text is a database concern, not a UI one.
...
export default function CorporatePage() {
  ...
  return (
    <div className="min-h-screen bg-white text-oxynavy-950">
      <Seo routeKey="corporate" />
      <Breadcrumbs
        jsonLdOnly
        items={[
          { label: t("common.breadcrumb.home"), to: path("home") },
          { label: t("corporate.hero.title") },
        ]}
      />
      <Header />
      <main>
...
function CorporateHero() {
  ...
  const title = settings?.["corporate_hero_title"] || t("corporate.hero.title");
  const description = settings?.["corporate_hero_description"] || t("corporate.hero.description");
  ...
  return (
    <section className="relative isolate overflow-hidden bg-oxynavy-950 text-white">
      <img src={imageUrl} alt={t("corporate.hero.imageAlt")} className="absolute inset-0 h-full w-full object-cover opacity-65" />
      ...
      <div className="relative mx-auto flex min-h-[320px] max-w-7xl items-center px-4 py-14 sm:px-6 lg:min-h-[390px] lg:px-8">
        <div className="max-w-2xl">
          <p className="text-xs font-extrabold tracking-[0.22em] text-white/70">{t("corporate.hero.eyebrow")}</p>
    ...
    <section className="bg-white py-16 sm:py-20">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl">
          <p className="text-xs font-extrabold tracking-[0.16em] text-oxynavy-700">{t("corporate.about.eyebrow")}</p>
...
function CorporateStats() {
  ...
  const settings = rawSettings as Record<string, string> | undefined;
  const stats = [
    { value: settings?.["corporate_years_experience"] || "15+", label: t("corporate.stats.years"), Icon: Building2 },
    { value: settings?.["corporate_completed_projects"] || "200+", label: t("corporate.stats.projects"), Icon: Award },
    { value: settings?.["corporate_export_countries"] || "10+", label: t("corporate.stats.exports"), Icon: Globe2 },
  ];

  return (
    <section className="bg-oxynavy-900 text-white">
      {isError && <div className="bg-red-900/40 px-4 py-2 text-center text-xs text-red-100">{t("corporate.stats.error")}</div>}
