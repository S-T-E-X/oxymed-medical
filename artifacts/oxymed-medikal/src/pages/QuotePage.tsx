  ...
  return (
    <section className="relative isolate overflow-hidden border-t border-steel-100 bg-steel-50">
      <ImageSlot
        ...
        alt={t("quote.hero.imageAlt")}
        className="absolute inset-y-0 start-0 hidden w-[58%] object-cover object-[60%_center] lg:block"
      />
      <div className="absolute inset-0 bg-gradient-to-r from-white/90 from-[0%] via-white/55 via-[30%] to-white/15" />
      <div className="absolute inset-y-0 end-0 hidden w-[44%] bg-white/92 backdrop-blur-[2px] lg:block" />
        ...
        <aside className="flex flex-col justify-center pt-2 lg:pt-4 lg:pb-4">
          ...
          <div className="mt-10 space-y-8">
            {quoteBenefits.map((benefit) => {
              ...
              return (
                <article key={benefit.key} className="flex max-w-[400px] items-start gap-5">
                  ...
                  <div className="pt-1">
                    <h2 className="text-[13px] font-extrabold text-oxynavy-950">
                      {t(`quote.benefits.${benefit.key}.title`)}
                    </h2>
                    <p className="mt-1.5 text-[13px] leading-5.5 text-oxynavy-950/72">
                      {t(`quote.benefits.${benefit.key}.description`)}
                    </p>
                  </div>
                </article>
              );
          ...
          <div className="min-w-0 flex-1 relative">
            <div className="quote-manhead-mobile">
              <img
                ...
                alt=""
                aria-hidden="true"
                className="pointer-events-none absolute -top-[101px] end-3 h-28 w-auto select-none object-contain"
              />
            </div>
            <QuoteForm />
          </div>
...
function QuoteForm() {
  ...
  const createMut = useCreateQuote({
    mutation: {
      ...
      onError: () => {
        setApiError(t("quote.form.apiError"));
      },
    },
  });
  ...
  function validate(): FieldErrors {
    ...
    if (!form.fullName.trim()) errors.fullName = t("quote.form.validation.fullNameRequired");
    if (!form.email.trim()) {
      errors.email = t("quote.form.validation.emailRequired");
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) {
      errors.email = t("quote.form.validation.emailInvalid");
    }
    if (!form.phone.trim()) errors.phone = t("quote.form.validation.phoneRequired");
    return errors;
  }
    ...
    return (
      <div className="flex items-center justify-center rounded-2xl border border-steel-100 bg-white px-8 py-16 shadow-[0_24px_56px_rgba(2,20,35,0.10)]">
        <div className="text-center">
          ...
          <h2 className="mt-5 text-2xl font-extrabold text-oxynavy-950">{t("quote.success.title")}</h2>
  ...
  return (
    ...
    >
      <div className="border-b border-steel-100 pb-5">
        <h2 className="text-[22px] font-extrabold leading-tight text-oxynavy-950">
          {t("quote.form.title")}
        </h2>
        <p className="mt-1.5 text-[13px] text-steel-600">{t("quote.form.subtitle")}</p>
      </div>

      <div className="mt-6 grid gap-x-5 gap-y-4 md:grid-cols-3">
        <Field
          label={t("quote.form.fields.fullName")}
          placeholder={t("quote.form.fields.fullNamePlaceholder")}
          value={form.fullName}
          onChange={(v) => set("fullName", v)}
          error={fieldErrors.fullName}
        />
        <Field
          label={t("quote.form.fields.email")}
          placeholder={t("quote.form.fields.emailPlaceholder")}
          type="email"
          value={form.email}
        ...
        />
        <Field
          label={t("quote.form.fields.phone")}
          placeholder={t("quote.form.fields.phonePlaceholder")}
          type="tel"
          value={form.phone}
      ...
      <div className="mt-4 grid gap-x-5 gap-y-4 md:grid-cols-2">
        <Field
          label={t("quote.form.fields.company")}
          placeholder={t("quote.form.fields.companyPlaceholder")}
          value={form.company}
          onChange={(v) => set("company", v)}
        />
        <Field
          label={t("quote.form.fields.jobTitle")}
      ...
      <label className="mt-4 block">
        <span className="mb-1.5 block text-[12px] font-semibold text-oxynavy-950">
          {t("quote.form.fields.notes")}
        </span>
        <textarea
          rows={3}
          placeholder={t("quote.form.fields.notesPlaceholder")}
          value={form.notes}
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
        <label className="block">
          ...
          <span className="flex h-11 cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-steel-200 bg-steel-50/60 px-4 text-[12px] text-steel-600 transition hover:border-oxynavy-400 hover:bg-oxynavy-50">
            ...
            {t("quote.form.fields.fileButton")}
            <input className="sr-only" type="file" accept=".pdf,.dwg,.jpg,.jpeg,.png" />
          </span>
          <span className="mt-1.5 block text-center text-[11px] text-steel-400">
            {t("quote.form.fields.fileHint")}
          </span>
        </label>
        ...
        >
          {createMut.isPending ? t("quote.form.submitting") : t("quote.form.submit")}
