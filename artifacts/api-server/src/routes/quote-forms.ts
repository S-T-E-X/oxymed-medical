...
// ─── Download PDF directly (server-side, puppeteer) ───────────────────────
router.get("/quote-forms/:id/pdf", requireAuth, async (req, res): Promise<void> => {
  const id = parseId(req.params["id"]!);
  const [form] = await db.select().from(quoteForms).where(eq(quoteForms.id, id)).limit(1);
  if (!form) { res.status(404).json({ error: "Teklif formu bulunamadı" }); return; }
  ...
  try {
    ...
    res.set({
      ...
      "Content-Length": String(rawPdf.length),
      "Content-Disposition": `attachment; filename="${form.quoteNo || "teklif"}.pdf"`,
    });
    res.send(Buffer.from(rawPdf));
  } catch (pdfErr) {
    if (pdfBrowser) { try { await pdfBrowser.close(); } catch { /* ignore */ } pdfBrowser = undefined; }
    req.log.error({ err: pdfErr }, "Quote PDF generation failed");
    res.status(500).json({ error: "PDF oluşturulamadı", detail: pdfErr instanceof Error ? pdfErr.message : String(pdfErr) });
  }
});
